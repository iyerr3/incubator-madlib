...
def _run_sql_query(sql, show_error):
    """
    ...
    """
    return _intenal_run_query(sql, show_error, con_args)
# ------------------------------------------------------------------------------


def _intenal_run_query(sql, show_error, con_args=con_args):
    # Define sqlcmd
    sqlcmd = 'psql'
...
def _get_madlib_dbrev(schema):
    ...
    """
    try:
        row = _run_sql_query("SELECT count(*) AS cnt FROM pg_tables " +
                             "WHERE schemaname='" + schema + "' AND " +
                             "tablename='migrationhistory'", True)
        if int(row[0]['cnt']) > 0:
            row = _run_sql_query("""SELECT version FROM %s.migrationhistory
...
def _get_dbver():
    ...
    try:
        versionStr = _run_sql_query("""SELECT pg_catalog.version()""",
                                    True)[0]['version']
        if portid == 'postgres':
...
def _check_db_port(portid):
    ...
    # Postgres
    try:
        row = _run_sql_query("SELECT version() AS version", True)
    except:
...
def _plpy_check(py_min_ver):
    ...
    rv = _run_sql_query("SELECT count(*) AS CNT FROM pg_language "
    ...
    if int(rv[0]['cnt']) > 0:
        ...
        try:
            _run_sql_query("CREATE LANGUAGE plpythonu;", True)
        except:
            _error('Cannot create language plpythonu. Stopping installation...', False)
            raise Exception

    # Check PL/Python version
    _run_sql_query("DROP FUNCTION IF EXISTS plpy_version_for_madlib();", False)
    _run_sql_query("""
        ...
        LANGUAGE plpythonu;
    """, True)
    rv = _run_sql_query("SELECT plpy_version_for_madlib() AS ver;", True)
    python = rv[0]['ver']
...
def _db_install(schema, dbrev, testcase):
    ...
    try:
        _run_sql_query("CREATE TABLE %s.__madlib_test_table (A INT);" % schema, False)
        _run_sql_query("DROP TABLE %s.__madlib_test_table;" % schema, False)
...
def _db_rename_schema(from_schema, to_schema):
    ...
    try:
        _run_sql_query("ALTER SCHEMA %s RENAME TO %s;" % (from_schema, to_schema), True)
    except:
...
def _db_create_schema(schema):
    ...
    try:
        _run_sql_query("CREATE SCHEMA %s;" % schema, True)
    except:
...
def _db_create_objects(schema, old_schema, upgrade=False, sc=None, testcase="",
    ...
    try:
        ...
        _run_sql_query("INSERT INTO %s.migrationhistory(version) "
...
def _db_rollback(drop_schema, keep_schema):
    ...
    try:
        _run_sql_query("DROP SCHEMA %s CASCADE;" % (drop_schema), True)
    except:
...
def main(argv):
  ...
  """)
    ...
    if args.command[0] in ('uninstall', 'reinstall') and (portid != 'hawq' or is_hawq2):
        ...
        affected_objects = _run_sql_query("""
            SELECT
        ...
        if go == 'N':
            ...
            try:
                _run_sql_query("DROP SCHEMA %s CASCADE;" % (schema), True)
            except:
    ...
    if args.command[0] == 'install-check':
        ...
        try:
            _run_sql_query("DROP USER IF EXISTS %s;" % (test_user), False)
        except:
        ...
        for moduleinfo in portspecs['modules']:
            ...
            _run_sql_query("DROP SCHEMA IF EXISTS %s CASCADE; CREATE SCHEMA %s;"
            ...
            _run_sql_query("DROP SCHEMA IF EXISTS %s CASCADE;" % (test_schema), True)
